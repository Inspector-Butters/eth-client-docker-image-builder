...
END
  ...
  "bazel")
    ...
    if ! $HOME/go/bin/bazelisk build --//tools:disable_progressive_merkleization=false //cmd/validator:validator --config=release --define pgo_enabled=0 --enable_bzlmod=false --remote_cache=grpcs://bazel-remote-cache-grpc.primary.production.platform.ethpandaops.io:443; then
      echo "Build failed with remote cache, trying without remote cache..."
      # Try without remote cache to avoid cache corruption issues
      if ! $HOME/go/bin/bazelisk build --//tools:disable_progressive_merkleization=false //cmd/validator:validator --config=release --define pgo_enabled=0 --enable_bzlmod=false; then
        ...
        $HOME/go/bin/bazelisk build --//tools:disable_progressive_merkleization=false //cmd/validator:validator --config=release --define pgo_enabled=0 --enable_bzlmod=false
      fi
    fi
